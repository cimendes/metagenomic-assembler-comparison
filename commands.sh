...
#Jul 2020
for file in $(ls *.fasta); do filename="${file%%.*}"; minimap2 --cs -t 4 -r 10000 -g 10000 -x asm20 --eqx --secondary=no /home/ines/Git/metagenomic-assembler-comparison/data/references/Zymos_Genomes_triple_chromosomes.fasta $file > ${filename}.paf; done
